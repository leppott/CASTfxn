...
getStressorList <- function(TargetSiteID, site.Clusters, chem.info, cluster.chem
  ...
  for (g in 1:numgps) {    # Generate 1 box plot for each group, ref sites in blue, target site in red
    ...
    n <- length(gpcoolvar)
    if(n>0) { ##FOR.n.START
      jpeg(filename = paste0("Results/boxes.example.",TargetSiteID
      ...
      plot(y= 1:n, x= runif(n,0,1), axes = F, type="n", xlab = "", ylab ="",
           xlim = c(0,1), cex.lab = 0.8)
      title(xlab=labx, line = 1, cex.lab = 0.8)
      # axis(1, at = seq(0,1, 0.2),labels = seq(0,1, 0.2))
      axis(2, at = 1:n, labels = gpcoolvar[1:n], las =1, cex.axis = 0.6)
      for(i in 1:n) {
        xvar <- cluster.chem[,gpcoolvar[i]]; dif <- diff(range(xvar, na.rm =T))
        newvar <- (xvar-min(xvar, na.rm=T))/dif
        boxplot(newvar, at = i,boxwex=0.5, horizontal =T, add =T,axes = F
        ...
        if (length(good.ref.data) != 0) {
          ...
          points(point2, rep(i,length(point2)), col = "blue", pch = 15,cex=0.6, bg = 2)
        }
        point1 <- (site.chem[,gpcoolvar[i]]-min(xvar, na.rm=T))/dif 
        points(point1, rep(i,length(point1)), col = "red", pch = 19,cex=0.6, bg = 2)
      }
      box(bty="l")
      dev.off()
    }
  ...
  write.table(data.chem.pctrank, file = paste("Results/chem.pctrank.",
