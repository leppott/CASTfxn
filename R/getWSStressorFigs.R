...
getWSStressorFigs <- function(TargetSiteID = TargetSiteID,
  ...
  elev_yr <- temp |>
    ...
    dplyr::summarize(
      Year_Min = min(Year),
      Year_Max = max(Year),
      WatershedValueMedian_Min = min(WatershedValueMedian) |> round(2) |> format(big.mark = ","),
      WatershedValueMedian_Max = max(WatershedValueMedian) |> round(2) |> format(big.mark = ","),
      TargetValue_Min = min(TargetValue) |> round(2)|> format(big.mark = ","),
      TargetValue_Max = max(TargetValue)|> round(2)|> format(big.mark = ",")) |>
  ...
  elev_no_yr <- temp |>
    ...
    dplyr::select(StreamCatVar, Year, WatershedValueMedian, TargetValue) |>
    dplyr::mutate(WatershedValueMedian = WatershedValueMedian |> round(2) |> format(big.mark = ","),
                  TargetValue = TargetValue |> round(2) |> format(big.mark = ",")) |>
