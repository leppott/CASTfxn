...
#' @description
#' Returns StreamCat watershed summary data for reaches in the specified state and, if provided in the cluster data file, 
#' in the 300 m buffer around the state outline. 
#' 
...
getStreamCatData <- function(localdir = "", dir_data = "", state = ""){
  ...
  data_stressorinfoWS <- data_stressorinfoWS %>%
    dplyr::mutate(Year = suppressWarnings(
      dplyr::case_when(
        ...
        grepl("^\\w*\\d{4}$", SCmetrics) ~ as.integer(sub("^\\w*(\\d{4})$", "\\1", SCmetrics)),
        TRUE ~ NA_integer_
      )
    ))
  
  data_stressorinfoWS <- data_stressorinfoWS %>%
    dplyr::mutate(StreamCatVar = suppressWarnings(dplyr::case_when(SCmetrics == "popden2010" ~ "popden2010",
  ...
  # Obtain actual data
  SCmetrics_nni <- data_stressorinfoWS %>% dplyr::filter(stringr::str_detect(SCmetrics, "_")) %>% dplyr::pull(SCmetrics) %>% paste(collapse = ",")
  SCmetrics_other <- data_stressorinfoWS %>% dplyr::filter(stringr::str_detect(SCmetrics, "_")== FALSE) %>% dplyr::pull(SCmetrics) %>% paste(collapse = ",")
  ...
  data_stressorWS_other <- StreamCatTools::sc_get_data(metric = SCmetrics_other,
    ...
    data_stressorWS <- full_join(data_stressorWS_nni, data_stressorWS_other, by = "comid")
    
    if("ReachLoc" %in% names(data_cluster)){
      buffer <- data_cluster %>% filter(ReachLoc == "Buffer") %>% pull(COMID) %>% paste(collapse = ",")
      ...
      buffer_other <- StreamCatTools::sc_get_data(metric = SCmetrics_other,
                                                aoi = 'watershed',
                                                comid = buffer) 
      buffer_WS <- full_join(buffer_nni, buffer_other, by = "comid")
      
      data_stressorWS <- data_stressorWS %>% bind_rows(buffer_WS)
      
      data_cluster <- data_cluster %>% dplyr::select(-ReachLoc)
    }
