...
getSiteMap <- function(sp_outline, sp_flowline, allSites, TargetSiteID
    ...
    site_map <- ggplot2::ggplot(data=sp_outline, fill=NA, color=col_outline
                                 , lwd=lwd_outline) +
        ggplot2::geom_sf(data=sp_flowline, inherit.aes=FALSE, color=col_flowline
                         , lwd=lwd_flowline) +
        ggplot2::geom_sf(data=sp_sites, inherit.aes=FALSE, color=col_sites_all
                         , pch=pch_sites_all, size=cex_sites_all) +
        ggplot2::geom_sf(data=sp_clustsites, inherit.aes=FALSE, color=col_sites_cl
                         , pch=pch_sites_cl, size=cex_sites_cl) +
        ggplot2::geom_sf(data=sp_refsites, inherit.aes=FALSE, color=col_sites_ref
                         , pch=pch_sites_ref, size=cex_sites_ref) +
        ggplot2::geom_sf(data=sp_targetsite, inherit.aes=FALSE, color=col_sites_targ
                         , pch=pch_sites_targ, size=cex_sites_targ) +
        ggplot2::geom_sf(data=sp_outline, inherit.aes=FALSE, fill=NA, color=col_outline
                         , lwd=lwd_outline) +
    ...
    site_map <- site_map +   # Build legend
        ggplot2::geom_rect(ggplot2::aes(xmin=-119.5, xmax=-118.5, ymin=32.5, ymax=33.5)
                           , color=col_outline, fill="white", lwd=lwd_outline) +
        ggplot2::annotate(geom="text", x=-119.25, y=33.4, label="Legend", size=size_legtitle) +
        ggplot2::annotate(geom="segment", x=-119.4, xend=-119.3, y=33.2, yend=33.2
                          , color=col_outline, lwd=lwd_outline) +
        ggplot2::annotate(geom="segment", x=-119.4, xend=-119.3, y=33.09, yend=33.09
                          , color=col_flowline, lwd=lwd_outline) +
        ggplot2::geom_point(ggplot2:::aes(x=-119.35, y=32.972), color=col_sites_all
                            , pch=pch_sites_all, size=1.4) +
        ggplot2::geom_point(ggplot2:::aes(x=-119.35, y=32.867), color=col_sites_cl
                            , pch=pch_sites_cl, size=1.4) +
        ggplot2::geom_point(ggplot2:::aes(x=-119.35, y=32.760), color=col_sites_ref
                            , pch=pch_sites_ref, size=1.4) +
        ggplot2::geom_point(ggplot2:::aes(x=-119.35, y=32.653), color=col_sites_targ
                            , pch=pch_sites_targ, size=1.4) +
        ggplot2::annotate(geom="text", x=-119.03, y=33.2, label="Study boundary"
                          , size=size_legelement) +
        ggplot2::annotate(geom="text", x=-119.12, y=33.09, label="Reaches"
                          , size=size_legelement) +
        ggplot2::annotate(geom="text", x=-119.135, y=32.972, label="All sites"
                          , size=size_legelement) +
        ggplot2::annotate(geom="text", x=-119.07, y=32.865, label="Cluster sites"
                          , size=size_legelement) +
        ggplot2::annotate(geom="text", x=-119.0256, y=32.763, label="Reference sites"
                          , size=size_legelement) +
        ggplot2::annotate(geom="text", x=-119.095, y=32.662, label="Target site"
                          , size=size_legelement)
