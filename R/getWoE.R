...
getWoE <- function(TargetSiteID,
  ...
  LoEcols <- c("StationID", "StressSampleID", "RespSampleID", "Response",
               "ResponseValue", "Quality", "Stressor", "StressorValue", "n",
               "nType", "Score", "LoEtrim", "LoE", "Analysis", "InOut", "biocomm")

  # Filenames for all files containing scores
  fnTSScores <- paste0(TargetSiteID, "_", toupper(biocomm), "_TS_Scores.tab")
  fnCOScores <- paste0(TargetSiteID, "_", toupper(biocomm), "_CO_Scores.tab")
  fnSRLogScores <- paste0(TargetSiteID, "_", toupper(biocomm), "_SRLog_Scores.tab")
  fnSRScores <- paste0(TargetSiteID, "_", toupper(biocomm), "_SRLin_Scores.tab")
  fnVPScores <- paste0(TargetSiteID, "_", toupper(biocomm), "_VP_Scores.tab")

  # Base data = dfQual
  # TargetSiteID, StressSampleID, RespSampleID, Index Score, BioNarrative, and
  # Biodegraded (Y/N)
  ...
  numSamps <- length(unique(as.character(dfStr$StressSampleID)))
  dfStr <- dfStr %>%
  ...
  df_allStrLoE <- merge(stressornames, LoEnames, by = "index", all = TRUE) %>%
    dplyr::select(!index) %>%
    dplyr::mutate(StationID = TargetSiteID) %>%
  ...
  for (l in 1:nrow(dfLoE)) {
    ...
    dfTemp <- dfStr
    dfTemp <- merge(dfStr, dfQual[, c("RespSampleID", index)],
                    by.x = "RespSampleID", by.y = "RespSampleID")
    ...
    if (booUse == 0) { # LoE was not evaluated. Need to enter NE scores for given LoE
      ...
      if (chrLoE == "TS") {
        dfTemp <- dfTemp %>%
          dplyr::mutate(n = numSamps,
                        ...
                        biocomm = biocomm) %>%
          dplyr::select(StationID, StressSampleID, RespSampleID,
      ...
      gaps <- cbind.data.frame("getWoE", chrLoE, 0, gapcomment)
      ...
      if (chrLoE == "CO") {
        if (file.exists(file.path(dirLoE, fnCOScores))) {
          dfCO <- read.table(file.path(dirLoE, fnCOScores), header = TRUE,
                             sep = "\t", stringsAsFactors = FALSE)
          colnames(dfCO) <- c("StationID", outcaseLabel, "StressSampleID",
                              "RespSampleID", index, "Quality",
          ...
          dfCO <- dfCO %>%
            dplyr::mutate(Response = index,
                          ...
                          Analysis = "Box plot",
                          InOut = "Inside the case") %>%
            dplyr::rename(ResponseValue = all_of(index), Score = Sc_Boxplot)
      ...
      if (chrLoE == "SRLog") {
        if (file.exists(file.path(dirLoE, fnSRLogScores))) {
          dfSRLog <- read.table(file.path(dirLoE, fnSRLogScores), header = TRUE,
                                sep = "\t", stringsAsFactors = FALSE)
          colnames(dfSRLog) <- c("StationID", "StressSampleID", "RespSampleID",
          ...
          dfSRLog <- dfSRLog %>%
            dplyr::mutate(Response = index,
                          ...
                          Analysis = "Logistic regression",
                          InOut = "Inside the case") %>%
            dplyr::rename(ResponseValue = all_of(index), Score = SC_SRLog)
      ...
      if (chrLoE == "SRLin") {
        if (file.exists(file.path(dirLoE,fnSRScores))) {
          dfSR <- read.table(file.path(dirLoE, fnSRScores), header = TRUE,
                             sep = "\t", stringsAsFactors = FALSE,
                             comment.char = "")

          colnames(dfSR) <- c("StationID", "Stressor", "Response",
                              "StressSampleID", "RespSampleID", "Quality",
      ...
      if (chrLoE == "VP") {
        if (file.exists(file.path(dirLoE,fnVPScores))) {
          dfVP <- read.table(file.path(dirLoE, fnVPScores), header = TRUE,
                             sep = "\t", stringsAsFactors = FALSE)
          colnames(dfVP) <- c("StationID", "RespSampleID",
                              "IndexValue", "Quality", "StressSampleID",
  ...
  coreData <- dfEvidenceLong %>%
    dplyr::distinct(StationID, StressSampleID, RespSampleID, Response,
                    Stressor, StressorValue, biocomm)

  dfEvidCurrent <- dfEvidenceLong %>%
    dplyr::distinct(Stressor, LoEtrim, StationID, LoE, nType, Analysis, InOut)

  dfEvidNE <- setdiff(df_allStrLoE, dfEvidCurrent)
  if (nrow(dfEvidNE) > 0) {
    dfEvidNE <- merge(coreData, dfEvidNE, by = c("StationID", "Stressor"))
  ...
  dfRank <- dplyr::select(dfRank, -StressSampleDate) %>%
    tidyr::pivot_longer(cols = !c(StationID, StressSampleID),
                        names_to = "Stressor",
  ...
  dfStrGpRank <- unique(merge(dfRank,
                              ...
                              by.x = "Stressor", by.y = "Stressor"))

  dfStrGpRankQual <- unique(merge(dfStrGpRank, dfQual,
                                  by.x = c("StationID", "StressSampleID"),
                                  by.y = c("StationID", "StressSampleID")))
  ...
  dfStrGpRankModl <- dfStrGpRank[grepl("_modeledflow", dfStrGpRank$StressSampleID), ]
  dfQualModl <- dfQual %>%
    dplyr::select(StationID, OutcaseCol, RespSampleID, all_of(index), BioDeg,
                  BioNarrative, ComparatorYN, BetterThan, OutsideCaseYN)
  dfStrGpRankQualModl <- unique(merge(dfStrGpRankModl, dfQualModl))
  dfStrGpRankQualModl <- dplyr::select(dfStrGpRankQualModl, StationID,
                                       -StressSampleID, Stressor, StressorPctRank,
                                       StressorType, OutcaseCol, RespSampleID,
  ...
  dfStrGpRankQualModl <- merge(dfStrGpRankQualModl, dfSampDates,
                               by.x = c("StationID", "RespSampleID"),
                               by.y = c("StationID", "RespSampleID"))
  dfStrGpRankQualModl <- dplyr::select(dfStrGpRankQualModl, StationID,
                                       StressSampleID, Stressor, StressorPctRank,
                                       StressorType, OutcaseCol, RespSampleID,
                                       all_of(index), BioDeg, BioNarrative,
  ...
  dfEvidenceLong <- dfEvidenceLong %>%
    ...
    dplyr::select(StationID, all_of(outcaseColName), BioComm, RespSampleID,
                  all_of(index), BioDeg, BioNarrative, Response, ResponseValue,
                  StressSampleID, StressorType, Label, Stressor, StressorValue,
  ...
  dfEvidenceLong <- dfEvidenceLong %>%
    dplyr::mutate(Score = ifelse(is.na(Score), "NE", Score),
                  Finding = case_when(Score == "NE" ~ "Not evaluated",
                                      ...
                                      TRUE ~ "Unknown")) %>%
    dplyr::arrange(StressorType, Label, StressSampleID, LoE)
  ...
  dfEvidenceWide <- dfEvidenceLong %>%
    dplyr::distinct(StressSampleID, Label, Stressor, StressorValue, Score, LoEtrim)
  ...
  dfEvidenceCounts <- dfEvidenceLong %>%
    dplyr::select(RespSampleID, StressSampleID, Label, Stressor, StressorValue,
                  Inside_Outside, Finding, Score) %>%
    dplyr::group_by(RespSampleID, StressSampleID, Label, Stressor, StressorValue,
                    Inside_Outside, Finding, Score) %>%
  ...
  dfEvidCountsWide <- dfEvidenceCounts %>%
    ...
    dplyr::group_by(StressSampleID, Label, Stressor, StressorValue) %>%
  ...
  dfEvidCountsWide <- merge(dfEvidenceWide, dfEvidCountsWide,
                            by.x = c("StressSampleID", "Label", "Stressor",
                                     "StressorValue"),
                            by.y = c("StressSampleID", "Label", "Stressor",
                                     "StressorValue"))

  # Merge basic data back in to summary
  dfEvidBasic <- unique(dfEvidenceLong[, c("StationID", "Cluster", "BioComm",
                                           "RespSampleID", index, "BioDeg",
                                           "BioNarrative", "StressSampleID",
                                           "StressorType", "Label", "Stressor",
                                           "StressorValue", "StressorPctRank")])

  dfEvidCountsWide <- merge(dfEvidBasic, dfEvidCountsWide,
                            by.x = c("StressSampleID", "Label", "Stressor",
                                     "StressorValue", "RespSampleID"),
                            by.y = c("StressSampleID", "Label", "Stressor",
                                     "StressorValue", "RespSampleID"))
  # Get Sample Dates
  ...
  dfEvidCountsWide <- dfEvidCountsWide[, c("StationID", "Cluster", "BioComm",
                                           "RespSampleID", "RespSampleDate", index,
                                           "BioDeg", "BioNarrative", "StressSampleID",
                                           "StressSampleDate", "StressorType", "Label",
                                           "Stressor", "StressorValue", "StressorPctRank",
                                           LoEcolnames, colNamesLoEcounts, "TotSupport",
                                           "TotRefute", "TotIndet", "TotNotEval", "WoE")] %>%
    dplyr::arrange(BioDeg, RespSampleDate, StressSampleDate, StressorType,
                   Label, StressorValue)
  ...
  dfData4ES <- unique(dfEvidCountsWide[, c("StationID", "BioComm", "BioDeg",
                                           index, as.character("RespSampleID"),
                                           as.character("StressSampleID"),
                                           "StressorType", "Stressor",
                                           LoEcolnames, "WoE")]) %>%
    dplyr::mutate(WoEnumeric = ifelse(WoE == "Supports", 1,
                                      ifelse(WoE == "Refutes", -1, 0))) %>%
    dplyr::select(-WoE) %>% dplyr::rename(WoE = WoEnumeric,
                                          IndexScore = all_of(index))
  dfData4ES <- dfData4ES %>%
    group_by(StationID, BioComm, BioDeg, StressorType) %>%
    dplyr::summarize(NumRespSamples = n_distinct(RespSampleID),
                     minIndex = min(IndexScore, na.rm = TRUE),
                     meanIndex = mean(IndexScore, na.rm = TRUE),
                     maxIndex = max(IndexScore, na.rm = TRUE),
                     NumStressSamples = n_distinct(StressSampleID),
