# updateAllScoresTable (Specific for SMC)
...
updateAllScoresTable <- function(dfAllScores, listWeights, fn_allscores
    ...
    indBioCond_rest <- c("wtd_pot_BioCondInd_rest", "wtd_pot_BioCxnInd"
                         , "wtd_pot_StressorInd", "wtd_pot_StressorCxnInd")
    indBioCond_prot <- c("wtd_pot_BioCondInd_prot", "wtd_pot_BioCxnInd"
                         , "wtd_pot_StressorInd", "wtd_pot_StressorCxnInd")
    indThreat <- c("wtd_thr_FireHazardInd", "wtd_thr_PlannedDevelopInd")
    indOpp <- c("wtd_opp_RecrInd", "wtd_opp_MSCPInd", "wtd_opp_NASVIInd"
                , "wtd_opp_UserDefInd")
    subsRPP_rest <- c("wtd_sub_PotentialSubIdx_rest", "wtd_sub_ThreatSubIdx"
                      , "wtd_sub_OppSubIdx")
    subsRPP_prot <- c("wtd_sub_PotentialSubIdx_prot", "wtd_sub_ThreatSubIdx"
                      , "wtd_sub_OppSubIdx")
    wts_potentialinds <- c("wt_pot_BioCondInd", "wt_pot_BioCxnInd"
                           , "wt_pot_StressorInd", "wt_pot_StressorCxnInd")
    wts_threatinds <- c("wt_thr_FireHazardInd", "wt_thr_PlannedDevelopInd")
    wts_oppinds <- c("wt_opp_RecrInd", "wt_opp_MSCPInd", "wt_opp_NASVIInd"
                     , "wt_opp_UserDefInd")
    ...
    dfAllScores <- dfAllScores %>%
        dplyr::mutate(wt_pot_BioCondInd = ifelse(is.na(pot_BioCondInd_rest)
                      ...
                      , wtd_opp_RecrInd = ifelse(is.na(opp_RecrInd), NA
                                            , signif((wt_opp_RecrInd*opp_RecrInd),3))
                      , wtd_opp_MSCPInd = ifelse(is.na(opp_MSCPInd), NA
                                            , signif((wt_opp_MSCPInd*opp_MSCPInd),3))
                      , wtd_opp_NASVIInd = ifelse(is.na(opp_NASVIInd), NA
                                            , signif((wt_opp_NASVIInd*opp_NASVIInd),3))
                      , wtd_opp_UserDefInd = ifelse(is.na(wt_opp_UserDefInd), NA
    ...
    dfAllScores$idx_RPPIndex_Rest <- ifelse(rowSums(is.na(dfAllScores[,subsRPP_rest]))==length(subsRPP_rest)
                                    , NA, signif(rowSums(dfAllScores[,subsRPP_rest]
                                        , na.rm = TRUE)/rowSums(dfAllScores[,wts_subs]
                                            , na.rm = TRUE),3))
    dfAllScores$idx_RPPIndex_Prot <- ifelse(rowSums(is.na(dfAllScores[,subsRPP_prot]))==length(subsRPP_prot)
                                    , NA, signif(rowSums(dfAllScores[,subsRPP_prot]
                                        , na.rm = TRUE)/rowSums(dfAllScores[,wts_subs]
                                            , na.rm = TRUE),3))
    ...
    restColNames <- c("idx_RPPIndex_Rest", "wtd_sub_PotentialSubIdx_rest"
                      , "wtd_pot_BioCondInd_rest")
    protColNames <- c("idx_RPPIndex_Prot", "wtd_sub_PotentialSubIdx_prot"
                      , "wtd_pot_BioCondInd_prot")
    otherColNames <- c("wtd_pot_BioCxnInd", "wtd_pot_StressorInd"
                       , "wtd_pot_StressorCxnInd", "wtd_sub_ThreatSubIdx"
                       , "wtd_thr_FireHazardInd", "wtd_thr_PlannedDevelopInd"
                       , "wtd_sub_OppSubIdx", "wtd_opp_RecrInd", "wtd_opp_MSCPInd"
                       , "wtd_opp_NASVIInd", "wtd_opp_UserDefInd")
