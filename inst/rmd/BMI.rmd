...
```{r bmiIndexSummary, out.width = "75%", echo = FALSE}
  ...
  if (file.exists(fn_bmiIndexGraphic)) {##IF.boo.pdf.START
    ...
    cat(msg_files)
    cat(paste0("<img src='", fn_bmiIndexGraphic, "'><br>"))
  } else {
...
region or a subset thereof. The following table includes whether a sample is considered degraded or not degraded
based on the cutoff value for the corresponding biological community index.
  
```{r bmiComparators, echo = FALSE}

  mycomps <- paste0(TargetSiteID, "_BMI_SiteQualities.csv")
  ...
  if (file.exists(fn_comps) == TRUE) {
    ...
    dfcomps <- read.csv(fn_comps, header = TRUE)
...
*Stressors marked as 'use in stressor id' in the stressor metadata and present in the stressor data.*

```{r bmiInitStressors, echo = FALSE}  
...
fn_init <- file.path(dir_results, TargetSiteID, paste(TargetSiteID, "InitialStressors.csv", sep = "_"))
initial_stressors <- read.csv(fn_init) |> dplyr::pull(Label)

  if (length(initial_stressors) > 0) {
    for (s in initial_stressors) {
      cat(paste0(bullet, " ", s, "<br>"))
    }
  } else {
    cat(paste0(bullet, " ", "No stressors were detected in any samples from the target site.", "<br>"))
  }

```
  
#### Stressor(s) eliminated: No paired-stressor response sample at the target site

*Stressors that were either never measured at the target site or not measured within the user-specified time window from a response sample*

```{r bmiElimStressorsNotMeas, echo = FALSE}
fn_elim <- file.path(dir_results, TargetSiteID, paste(TargetSiteID, "StressorsEliminated.csv", sep = "_"))
df_elim_bmi <- read.csv(fn_elim) |> 
  dplyr::filter(is.na(Biocomm) | Biocomm == "bmi")

elim_not_meas <- df_elim_bmi |> 
  dplyr::filter(Reason == "Not measured at target site") |> 
  dplyr::pull(Label)

  if (length(elim_not_meas) > 0) {
    for (s in elim_not_meas) {
      cat(paste0(bullet, " ", s, "<br>"))
    }
  } else {
    cat(paste0(bullet, " ", "No stressors were eliminated because there were no paired stressor-response samples at the target site (benthic macroinvertebrates).", "<br>"))
  }
```

#### Stressor(s) eliminated: Insufficient paired stressor-response samples across all comparator sites 

*Stressors with fewer paired stressor-response samples across all comparator sites than the user-specified sample limit*

```{r bmiElimStressorsInsuff, echo = FALSE}
elim_insuff <- df_elim_bmi  |>  
  dplyr::filter(Reason == "Insufficient paired samples") |> 
  dplyr::pull(Label)

  if (length(elim_insuff) > 0) {
    for (s in elim_insuff) {
      cat(paste0(bullet, " ", s, "<br>"))
    }
  } else {
    cat(paste0(bullet, " ", "No stressors were eliminated because there were insufficient paired stressor-response samples at all comparator sites (benthic macroinvertebrates).", "<br>"))
  }
```

#### Stressor(s) eliminated: By the comparison of target and unimpaired comparator sample values

*Stressors eliminated by the co-occurrence line of evidence*

```{r bmiElimStressorsCO, echo = FALSE}
elim_co <- df_elim_bmi |> 
    dplyr::filter(Reason == "Co-occurrence") |> 
    dplyr::pull(Label)
  ...
  if (length(elim_co) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated by the comparison of target and unimpaired comparator sample values (benthic macroinvertebrates).", "<br>"))
  }
```

#### Stressors remaining
```{r}
stressors_elim_any <- df_elim_bmi |> 
  dplyr::distinct(Label) |> 
  dplyr::pull(Label)

siteDetects <- setdiff(initial_stressors, stressors_elim_any)
  ...
  if (length(siteDetects) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors remaining (benthic macroinvertebrates).", "<br>"))
  }
```

<!-- ```{r bmiElimStressors, echo = FALSE} -->

<!--   dir.sub3 <- "_WoE" -->
<!--   fnNE <- paste0(TargetSiteID, "_BMI_DetectsNotEvalFurther.csv") -->
<!--   fnNE <- file.path(dir_results, TargetSiteID, "BMI", dir.sub3, fnNE) -->

<!--   if (file.exists(fnNE)) { -->
<!--     notEvaluated <- read.csv(fnNE, header = TRUE) -->
...
```{r bmiLoEscores, echo = FALSE}
  
  fnLoEbyStressor <- paste0(TargetSiteID, "_LoEs.csv")
  ...
  if (!file.exists(fnLoEbyStressor)) {# No WoE file
    ...
    dfLoEbyStressor <- read.csv(fnLoEbyStressor, na.strings = "NA",
    ...
    dfLoEbyStressor <- dfLoEbyStressor %>%
      ...
      dplyr::select(Stressor, RespSampleID, RespSampleDate, CO, Suff, 
                    BioGradIn, TS, VP_SSTV, VP_SSIbox, VP_SSIlog, 
                    BioGradOut) %>%
            # dplyr::select(Stressor, RespSampleID, RespSampleDate, CO, Suff, 
    ...
    if (report_format == "html") {
      ...
      sketch <- htmltools::withTags(table(class = 'display', thead(
        ...
        tr(lapply(
          c('SSTolVals', 'SSI Co-Occurrence', 'SSI Sufficiency'), th
        ))
      )))
      ...
      DT::datatable(dfLoEbyStressor,
        ...
        DT::formatStyle(columns = colnames(dfLoEbyStressor), `text-align` = "left") |> 
...
```{r bmiLoESummary, echo = FALSE}

  fnLoESummary <- paste0(TargetSiteID, "_LoESummary.csv")
 ...
 if (!file.exists(fnLoESummary)) {# No WoE file
   ...
   dfLoESummary <- read.csv(fnLoESummary, na.strings = "NA",
                                 header = TRUE, strip.white = TRUE,
                                 stringsAsFactors = FALSE)
...
```{r bmiLoESummaryFig, echo = FALSE}
fn_loe_fig <- file.path(dir_results, TargetSiteID, "BMI", "_WoE", paste0(TargetSiteID, "_BMI_LoESummaryFig.png"))
...
stressors <- siteDetects
#paramName <- data_stressInfo$StdParamName[data_stressInfo$Label %in% stressors] # this doesn't work because does not preserve the order of stressors
...
for (s in seq_along(stressors)) {
  ...
  stressName <- data_stressInfo |> dplyr::filter(Label == stressLabel) |> dplyr::pull(StdParamName)
  ...
  if (length(all.img) > 0) {
    ...
    if (length(all.img[grepl(paste0(bmiIndex, "_GI.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(bmiIndex, "_GI.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient inside-the-case image is not available.\n\n")
    }
    
    if (length(all.img[grepl(paste0(bmiIndex, "_GO.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(bmiIndex, "_GO.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient outside-the-case image is not available.\n\n")
    }
    
    # if (length(all.img[grepl(paste0(bmiIndex, "_TS.png$"), all.img)]) == 1) {
    #   fn <- all.img[grep(paste0(bmiIndex, "_TS.png$"), all.img)]
    #   cat(paste0("<img src='", fn, "'><br>"))
    # } else {
