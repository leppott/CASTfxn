...
```{r report_date, echo = FALSE}
  ...
  cat(paste("**Number of allowable lag days between stressor and response measurements:** -",
            lagdays, "\n\n", sep = ""))
  # cat(paste("**Cluster assignment file:** ", fn.cluster, "\n", sep = ""))
...
```{r photos, out.width = "50%", out.extra = 'style = "padding:10px"', echo = FALSE}
dir.sub <- "SiteInfo"
...
if (length(fn_all) > 0) {##IF.boo.pdf.START
  ...
  if (length(fn_Photos_jpg) == 0) {
    txt_NoFile <- "No site photos exist.\n"
    cat(txt_NoFile)
  } else {
    #
    for (p in seq_along(1:length(fn_Photos_jpg))) {
      fn_img <- fn_Photos_jpg[p]
      knitr::include_graphics(fn_img)
    }
    #
  }
...
if (nrow(dfSamps) > 0) {# Samples exist for target site
  ...
  dfSamps <- dplyr::select(dfSamps, !c(StationID, COMID, OutcaseCol, IncaseCol)) %>%
    ...
    dplyr::mutate(ColName = dplyr::case_when(ColName == "ChemistrySampleID" ~
                                               "Chemistry Sample ID",
                                             ColName == "HabitatSampleID" ~
                                               "Habitat Sample ID",
                                             ColName == "FieldSampleID" ~
                                               "Field Sample ID",
                                             ColName == "ModeledSampleID" ~
                                               "Modeled Sample ID",
                                             ColName == "BMISampleID" ~
                                               "BMI Sample ID",
                                             ColName == "AlgSampleID" ~
                                               "Algae Sample ID",
                                             ColName == "FishSampleID" ~
                                               "Fish Sample ID",
                                             TRUE ~ "Undefined Sample ID")) %>%
...
```{r bmiIdxGraph, out.width = "80%", out.extra = 'style = "padding:10px"', echo = FALSE}
dir.sub3 <- "SiteInfo"
...
```{r bmicompsitesummtab, echo = FALSE, out.extra = 'style = "padding:10px"'}
...
if ("bmi" %in% biocommlist) { # BMI were evaluated
  ...
  if (!file.exists(fnLoEbyStressor)) {# No WoE file
    ...
    if (report_format == "html") {
      ...
      tblCaption <- paste0("Detailed weight-of-evidence for each stressor ",
                           "and sample.")
...
```{r stressor.graphics, echo = FALSE, out.width = "75%", fig.align = "left"}
...
for (s in seq_along(paramName)) {
  ...
  all.img <- list.files(file.path(dir.biocomm, stressName), full.names = TRUE)
  
  if (length(all.img) > 0) {
    
    # fn_Plots_png <- all.img[grep(".png", all.img)]
    fn_img_CO <- all.img[grep(paste0("BMI_", bioIndex, "_CO.png"), all.img)]
    fn_img_SU <- all.img[grep(paste0("BMI_", bioIndex, "_SU.png"), all.img)]
    
    # ![text here](file.path(filename))
    knitr::include_graphics(fn_img_CO)
    knitr::include_graphics(fn_img_SU)
    
    # if (length(fn_Plots_png) == 0) {
    #   txt_NoFile <- "No stressor plots exist.\n"
    #   cat(txt_NoFile)
    # } else {
    #   #
    #   for (p in seq_along(1:length(fn_Plots_png))) {
    #     fn_img <- fn_Plots_png[p]
    #     knitr::include_graphics(fn_img_CO, fn_img_SU)
    #   }
    #   #
    # }
  ...
  # knitr::include_graphics(all.img)
