...
macroinvertebrates (`r ifelse(!is.null(bmiIndexGp), bmiIndexGp, "N/A")`),
algae (`r ifelse(!is.null(algIndexGp), algIndexGp, "N/A")`), or fish
(`r ifelse(!is.null(fishIndexGp), fishIndexGp, "N/A")`).
...
if (nrow(dfSamps) > 0) {# Samples exist for target site
  ...
  if (!is.null(data_bmiMetrics)) {
    ...
    dfbmi <- dplyr::filter(data_bmiMetrics, StationID == TargetSiteID) %>%
      dplyr::select(RespSampleID, all_of(bmiIndexGp), Quality)
...
# cluster figure
dir_clust_fig <- file.path(dir_results, TargetSiteID, dir.sub, "Photos")
# not in metadata, created in Check Files
path_clust_fig <- file.path(dir_clust_fig, "cluster_graphic.png")

if (file.exists(path_clust_fig) == TRUE) {
  knitr::include_graphics(path_clust_fig)
} else {
  cat("No file exists; cluster figure.\n")
}
```
