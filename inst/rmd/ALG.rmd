...
for (s in seq_along(paramName)) {
  ...
  if (length(all.img) > 0) {
    ...
    if (length(all.img[grepl(paste0(algIndexGp, "_GI.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(algIndexGp, "_GI.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient inside-the-case image is not available.\n\n")
    }
    
    if (length(all.img[grepl(paste0(algIndexGp, "_GO.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(algIndexGp, "_GO.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient outside-the-case image is not available.\n\n")
    }
    
    # if (length(all.img[grepl(paste0(algIndexGp, "_TS.png$"), all.img)]) == 1) {
    #   fn <- all.img[grep(paste0(algIndexGp, "_TS.png$"), all.img)]
    #   cat(paste0("<img src='", fn, "'><br>"))
    # } else {
