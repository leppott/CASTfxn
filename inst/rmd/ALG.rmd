...
```{r algComparators, echo = FALSE}
  ...
  mycomps <- paste0(TargetSiteID, "_ALG_SiteQualities.csv")
...
```{r algElimStressorsNotMeas, echo = FALSE}
fn_elim <- file.path(dir_results, TargetSiteID, paste(TargetSiteID, "StressorsEliminated.csv", sep = "_"))
df_elim_alg <- read.csv(fn_elim) |> 
  dplyr::filter(is.na(Biocomm) | Biocomm == "alg")

elim_not_meas <- df_elim_alg |> 
  ...
  if (length(elim_not_meas) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated because there were no paired stressor-response samples at the target site (algae).", "<br>"))
  }
```
...
```{r algElimStressorsInsuff, echo = FALSE}
elim_insuff <- df_elim_alg  |>  
  ...
  if (length(elim_insuff) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated because there were insufficient paired stressor-response samples at all comparator sites (algae).", "<br>"))
  }
```
...
```{r algElimStressorsCO, echo = FALSE}
elim_co <- df_elim_alg |> 
  ...
  if (length(elim_co) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated by the comparison of target and unimpaired comparator sample values (algae).", "<br>"))
  }
```

#### Stressors remaining
```{r}
stressors_elim_any <- df_elim_alg |> 
...
siteDetects <- setdiff(initial_stressors, stressors_elim_any)
  ...
  if (length(siteDetects) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors remaining (algae).", "<br>"))
  }
```
...
<!-- #  -->
<!-- #   dir.sub3 <- "_WoE" -->
<!-- #   fnNE <- paste0(TargetSiteID, "_ALG_DetectsNotEvalFurther.csv") -->
