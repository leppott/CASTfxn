...
```{r fishComparators, echo = FALSE}
  ...
  mycomps <- paste0(TargetSiteID, "_FISH_SiteQualities.tab")
...
```{r fishElimStressors, echo = FALSE}
  ...
  dir.sub3 <- "_WoE"
  fnNE <- paste0(TargetSiteID, "_FISH_DetectsNotEvalFurther.tab")
...
for (s in seq_along(paramName)) {
  ...
  if (length(all.img) > 0) {
    ...
    if (length(all.img[grepl(paste0(fishIndexGp, "_GI.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(fishIndexGp, "_GI.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient inside-the-case image is not available.\n\n")
    }
    
    if (length(all.img[grepl(paste0(fishIndexGp, "_GO.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(fishIndexGp, "_GO.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient outside-the-case image is not available.\n\n")
    }
    
    # if (length(all.img[grepl(paste0(fishIndexGp, "_TS.png$"), all.img)]) == 1) {
    #   fn <- all.img[grep(paste0(fishIndexGp, "_TS.png$"), all.img)]
    #   cat(paste0("<img src='", fn, "'><br>"))
    # } else {
