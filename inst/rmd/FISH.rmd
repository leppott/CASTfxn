...
```{r fishIndexSummary, out.width = "75%", echo = FALSE}

  dir.sub3 <- "SiteInfo"
  fn_fishIndexGraphic <- file.path(dir_results, TargetSiteID, dir.sub3, 
                                  paste0(TargetSiteID, 
                                         "_FISH_IndexBoxplotsByCase.png"))
  
  if (file.exists(fn_fishIndexGraphic)) {##IF.boo.pdf.START
    msg_files <- paste0("This image file is in the '",
                        paste0("Results/", dir.sub3, "' folder.\n\n"))
    cat(msg_files)
    cat(paste0("<img src='", fn_fishIndexGraphic, "'><br>"))
  } else {
    msg_files <- "The fish summary image file is not accessible."
    cat(msg_files)
  }
...
```{r fishComparators, echo = FALSE}

  mycomps <- paste0(TargetSiteID, "_FISH_SiteQualities.csv")
...
```{r fishInitStressors, echo = FALSE}  
...
```{r fishElimStressorsNotMeas, echo = FALSE}
fn_elim <- file.path(dir_results, TargetSiteID, paste(TargetSiteID, "StressorsEliminated.csv", sep = "_"))
df_elim_fish <- read.csv(fn_elim) |> 
  dplyr::filter(is.na(Biocomm) | Biocomm == "fish")

elim_not_meas <- df_elim_fish |> 
  ...
  if (length(elim_not_meas) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated because there were no paired stressor-response samples at the target site (fish).", "<br>"))
  }
```
...
```{r fishElimStressorsInsuff, echo = FALSE}
elim_insuff <- df_elim_fish  |>  
  ...
  if (length(elim_insuff) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated because there were insufficient paired stressor-response samples at all comparator sites (fish).", "<br>"))
  }
```
...
```{r fishElimStressorsCO, echo = FALSE}
elim_co <- df_elim_fish |> 
  ...
  if (length(elim_co) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors were eliminated by the comparison of target and unimpaired comparator sample values (fish).", "<br>"))
  }
```

#### Stressors remaining
```{r}
stressors_elim_any <- df_elim_fish |> 
...
siteDetects <- setdiff(initial_stressors, stressors_elim_any)
  ...
  if (length(siteDetects) > 0) {
    ...
    cat(paste0(bullet, " ", "No stressors remaining (fish).", "<br>"))
  }
```

<!-- # ```{r fishElimStressors, echo = FALSE} -->
<!-- #  -->
<!-- #   dir.sub3 <- "_WoE" -->
<!-- #   fnNE <- paste0(TargetSiteID, "_FISH_DetectsNotEvalFurther.csv") -->
<!-- #   fnNE <- file.path(dir_results, TargetSiteID, "FISH", dir.sub3, fnNE) -->
<!-- #    -->
<!-- #   if (file.exists(fnNE)) { -->
<!-- #     notEvaluated <- read.csv(fnNE, header = TRUE) -->
<!-- #     NE <- merge(notEvaluated, data_stressInfo, by.x = "FISH_NotEvaluated",  -->
...
```{r fishLoEscores, echo = FALSE}
  
  fnLoEbyStressor <- paste0(TargetSiteID, "_LoEs.csv")
  fnLoEbyStressor <- file.path(dir_results, TargetSiteID, "FISH", "_WoE", 
                               fnLoEbyStressor)
...
```{r fishLoESummary, echo = FALSE}

  fnLoESummary <- paste0(TargetSiteID, "_LoESummary.csv")
  fnLoESummary <- file.path(dir_results, TargetSiteID, "FISH", "_WoE", fnLoESummary)
...
```{r fishLoESummaryFig, echo = FALSE}
fn_loe_fig <- file.path(dir_results, TargetSiteID, "FISH", "_WoE", paste0(TargetSiteID, "_FISH_LoESummaryFig.png"))
...
```{r fish.LoE.graphics, echo = FALSE, out.width = "75%", fig.align = "left"}
...
dir.biocomm <- file.path(dir_results, TargetSiteID, "FISH")
...
for (s in seq_along(stressors)) {
  ...
  if (length(all.img) > 0) {
    ...
    if (length(all.img[grepl(paste0(fishIndex, "_GI.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(fishIndex, "_GI.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient inside-the-case image is not available.\n\n")
    }
    
    if (length(all.img[grepl(paste0(fishIndex, "_GO.png$"), all.img)]) == 1) {
      fn <- all.img[grep(paste0(fishIndex, "_GO.png$"), all.img)]
      cat(paste0("<img src='", fn, "'><br>"))
    } else {
      cat("\nBiological gradient outside-the-case image is not available.\n\n")
    }
    
    # if (length(all.img[grepl(paste0(fishIndex, "_TS.png$"), all.img)]) == 1) {
    #   fn <- all.img[grep(paste0(fishIndex, "_TS.png$"), all.img)]
    #   cat(paste0("<img src='", fn, "'><br>"))
    # } else {
